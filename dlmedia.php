...
if (isset($_POST['mid'])) {
    $media_id = intval($_POST['mid']);
} elseif (isset($_GET['mid'])) {
    $media_id = intval($_GET['mid']);
}
if (isset($_POST['lid'])) {
...
$media_size=0;
    ...
    if ($result) {
        while ($myrow=$xoopsDB->fetchArray($result)) {
            ...
            if ($media_lang_fileref == $myrow['media_file_id']) {
                break;
            }
        }
    } else {
    ...
    if (file_exists($file)) {
        if (substr($media_mimetype, 1, 6) == 'image/') {
            header('Content-Type: '.$media_mimetype);
        } else {
