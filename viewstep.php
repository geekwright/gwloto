...
if (isset($_POST['op_addstep']) || isset($_POST['add'])) {
    ...
    if (!isset($jobcplans[$currentplan])) {
        $op='add';
    } else {
        $err_message=_MD_GWLOTO_JOBSTEP_DUPLICATE_PLAN;
    }
}
if (isset($_POST['print'])) {
    $prtmsg = _MD_GWLOTO_JOB_PRINT_REDIR_MSG;
    if ($currentplan) {
        redirect_header("printjob.php?jid=$currentjob&cpid=$currentplan", 3, $prtmsg);
    } else {
        redirect_header("printjob.php?jid=$currentjob", 3, $prtmsg);
    }
}
if (!$user_can_edit) {
    $op='display';
...
if ($op=='update' || $op=='add') {
    ...
    if (isset($_POST['job_step_status'])) {
        $job_step_status = cleaner($_POST['job_step_status']);
    }
    if (!isset($stepstatus[$job_step_status])) {
        $job_step_status = 'planning';
    }
...
if ($user_can_edit) {
    ...
    if ($op=='add') {
        $formtitle=_MD_GWLOTO_JOBSTEP_NEW_FORM;
    } else {
        $formtitle=_MD_GWLOTO_JOBSTEP_EDIT_FORM;
    }
