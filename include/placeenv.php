...
// media specified
if (isset($_POST['mid'])) {
    $currentmedia = intval($_POST['mid']);
} elseif (isset($_GET['mid'])) {
    $currentmedia = intval($_GET['mid']);
}
...
if (isset($_POST['ptid'])) {
    $currentpoint = intval($_POST['ptid']);
} elseif (isset($_GET['ptid'])) {
    $currentpoint = intval($_GET['ptid']);
}
...
if (isset($_POST['jsid'])) {
    $currentjobstep = intval($_POST['jsid']);
} elseif (isset($_GET['jsid'])) {
    $currentjobstep = intval($_GET['jsid']);
}
...
if (isset($_POST['pid'])) {
    $currentplace = intval($_POST['pid']);
} elseif (isset($_GET['pid'])) {
    $currentplace = intval($_GET['pid']);
}

$places=array();

if (isset($defaultplace)) {
    unset($defaultplace);
}
...
if (isset($system_language)) {
    unset($system_langage);
}

if (isset($xoopsConfig['language'])) {
    $system_language=$xoopsConfig['language'];
}
if (isset($icmsConfig['language'])) {
    $system_language=$icmsConfig['language'];
}
if (isset($_SESSION['UserLanguage'])) {
    $system_language=$_SESSION['UserLanguage'];
}

if (isset($system_language)) {
    $langfolders=getLanguageByFolder();
    if (isset($langfolders[$system_language])) {
        $language=$langfolders[$system_language];
    }
}
...
if (isset($currentplace)) {
    ...
    if ($result) {
        ...
        if (count($tmppid)>0) {
            $places['choose']=getMultiPlaceNames($tmppid, $language);
        }
    }
...
if (isset($currentplace)) {
    $xoopsTpl->assign('currentplace', $currentplace);
}
if (isset($currentplan)) {
    $xoopsTpl->assign('currentplan',  $currentplan);
}
if (isset($currentpoint)) {
    $xoopsTpl->assign('currentpoint', $currentpoint);
}
if (isset($currentjob)) {
    $xoopsTpl->assign('currentjob',   $currentjob);
}
if (isset($currentmedia)) {
    $xoopsTpl->assign('currentmedia', $currentmedia);
}
if (isset($language)) {
    $xoopsTpl->assign('language', $language);
}
// end of common stuff
;
