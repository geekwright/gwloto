...
$mediasearchterms='';
$media_class='';
if (isset($_GET['mediasearchterms'])) {
    $mediasearchterms = cleaner($_GET['mediasearchterms']);
}
if (isset($_POST['mediasearchterms'])) {
    $mediasearchterms = cleaner($_POST['mediasearchterms']);
}
$xoopsTpl->assign('mediasearchterms', $mediasearchterms);
$mediasearchterms=dbescape($mediasearchterms);
if (isset($_GET['media_class'])) {
    $media_class = cleaner($_GET['media_class']);
}
if (isset($_POST['media_class'])) {
    $media_class = cleaner($_POST['media_class']);
}
...
// set up pagenav
    ...
    if ($total > $limit) {
        ...
        if ($mediasearchterms!='') {
            $extranav.="&mediasearchterms=$mediasearchterms";
        }
        if ($media_class!='') {
            $extranav.="&media_class=$media_class";
        }
...
// build an array of media items
    ...
    if (count($media)==0) {
        if ($searchsql=='') {
            $err_message=_MD_GWLOTO_MEDIA_NO_MEDIA;
        } else {
            $err_message=_MD_GWLOTO_MEDIA_NO_MATCH;
        }
    }
